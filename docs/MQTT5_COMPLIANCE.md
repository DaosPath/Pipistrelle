...
| Topic Alias | Implemented for QoS0 routing | Client→Server aliases support establish/use/remap semantics, zero-length Topic Name reuse, per-Network-Connection reset, range checks and protocol errors. Server→Client aliases are independently assigned when the peer advertises Topic Alias Maximum; the first PUBLISH establishes the mapping and later PUBLISH packets may reuse an empty Topic Name. |
...
| Topic Alias Maximum | Implemented | CONNACK advertises configurable `PIPISTRELLE_TOPIC_ALIAS_MAXIMUM` (default 32) and enforces it for Client→Server PUBLISH. CONNECT Topic Alias Maximum is honored as the upper bound for Server→Client alias assignment. |
...
- `test_protocol_v2.py` — raw MQTT v5 packet/state tests, including Application Message properties, Message Expiry, Topic Alias lifecycle/remap/reset in both directions, UNSUBSCRIBE, negotiated limits, assigned ClientIDs, TCP-fragmented CONNECT, UTF-8 and varint errors.
